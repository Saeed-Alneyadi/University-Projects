...
def linear_regression(X, Y):
    ...
    # Adding Row of Ones to X and store the new matrix in the X_ones
    X_ones = np.append(X, [1])
    idx = 0
    while idx < N - 1:
        X_ones = np.append(X_ones, 1)
        idx += 1
    X_ones = X_ones.reshape(D + 1, N)

    # Calculting the w~ using the Linear algebra formulation to obtain closed-form solution 
    XXT = np.matmul(X_ones, X_ones.transpose()) # X*X^T
    XXT_inv = np.linalg.inv(XXT) # (XX^T)^-1
    XY = np.matmul(X_ones, Y) # X*Y
    result = np.matmul(XXT_inv, XY) # (XX^T)^-1 * XY

    # Extracting w and b from w~
    w = result[:-1]
    b = result[-1]
