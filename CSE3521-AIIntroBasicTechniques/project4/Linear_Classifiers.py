...
def Logistic_Regression(X, Y, learningRate=0.01, maxIter=100):
    ...
    for t in range(maxIter):
        # Calculating the predicted probabilities using @np.matmul and @sigmoid functions
        z = np.matmul(w.transpose(), X)
        predicted_probabilities = sigmoid(z)

        error = predicted_probabilities - Y.T # Computing the error by subtracting @predicted_probabilities by @Y.T

        gradient = np.matmul(X, error.transpose()) / N # Calculating the gradient using @np.matmul with input of @X and the @transpose() of @error

        w = w - learningRate * gradient # Updating the weights @w by subtracting with matrix multiplication of @learningRate and @gradient
...
def Perceptron(X, Y, learningRate=0.01, maxIter=100):
    ...
    for t in range(maxIter):
        ...
        Y = Y[permutation, :]
        for n in range(N):
            prediction = np.matmul(w.transpose(), X[:, n]) # Calculating the prediction using @np.matmul

            if Y[n] * prediction <= 0: # Checking for Misclassification using @prediction and @Y
                # Updating the Weights @w using @X, @Y and @learningRate
                weight_update = learningRate * Y[n] * X[:, n].reshape(-1, 1)
                w = w + weight_update
