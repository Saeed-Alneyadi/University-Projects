...
class NaiveBayesClassifier(object):
    ...
    def train(self, training_sentences, training_labels):
        ...
        for sentence in range(X.shape[0]):
            for word, index in self.V.items():
                if word in training_set[sentence]:
                    X[sentence][index] = 1
                else:
                    X[sentence][index] = 0
        ...
        self.conditional = np.zeros((len(C),D))

        self.prior = [C[0]/N, C[1]/N, C[2]/N]
        # print(self.prior)

        # 0 -> 948 / 949 -> 1841 / 1842 -> 3096
        # print(training_sentences[0])
        # print(training_sentences[949])
        # print(training_sentences[1842])

        for cls in CCum:
            idx = CCum.index(cls)
            sentence_count = 0
            while sentence_count <= cls - 1:
                dim = 0
                self.conditional[idx][dim] = 1
                while dim < self.conditional.shape[1]:
                    self.conditional[idx][dim] += X[sentence_count][dim]

                    dim += 1
                sentence_count += 1
            self.conditional[idx] = [element / C[idx] for element in self.conditional[idx]]
    ...
    def predict(self, test_sentence):
        ...
        cls_log_probs = np.log10(self.prior)

        test_probs = np.copy(self.conditional)
        for idx in range(BOW.shape[1]):
            if BOW[0][idx] == 0:
                test_probs[:, idx] = 0

        # print(test_probs[test_probs > 0])

        # print(np.sum(np.log(BOW[BOW > 0])))

        sums_log_probs = np.zeros(test_probs.shape[0])

        for i in range(test_probs.shape[0]):
            sum_log_prob = 0
            for j in range(test_probs.shape[1]):
                if test_probs[i][j] != 0:
                    sum_log_prob += np.log10(test_probs[i][j])
            sums_log_probs[i] = sum_log_prob

        # print(sums_log_probs.shape)
        # print(cls_log_probs.shape)
        label_probability = np.roll(cls_log_probs, shift=1) + np.roll(sums_log_probs, shift=1)
