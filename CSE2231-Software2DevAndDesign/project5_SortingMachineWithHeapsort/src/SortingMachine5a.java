...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        if (2*top +1 < last) {
        if (order.compare(array[top], array[2*top+1]) < 0) {
        	exchangeEntries(array, top, 2 * top + 1);
            siftDown(array, 2 * top + 1, last, order);
        } else if (order.compare(array[top], array[2*top+2]) < 0) {
    ...
    public final void add(T x) {
        ...
        /*
         * Enqueue {@code x} to {@code this.entries}.
         */
    ...
    public final void changeToExtractionMode() {
        assert this.isInInsertionMode() : "Violation of: this.insertion_mode";
        /*
         * Set {@code this.insertionMode} to false.
         */
        this.insertionMode = false;
        /*
         * Creates a heap array using {@code buildHeap()} and store it to {@code
         * this.heap}.
         */
    ...
    public final T removeFirst() {
        ...
        /*
         * Replace the first entry in heap with the last one, decrement {@code
         * this.heapSize}, and call {@code heapify()} on {@code this.array} to
        ...
        assert this.conventionHolds();
        /*
         * RETURN Statement: Returning the removed element which is located out
         * of heap range.
         */
        return root;
    }
    ...
    public final int size() {
        /*
         * {@code contentSize} variable deceleration.
         */
        int contentSize;
        /*
         * If the StoringMachine Object is in insertion mode, the length of
         * {@code this.entries} will be assigned to {@code contentSize}.
         * Otherwise, {@code this.heapSize} will be assigned to it.
         */
        if (this.insertionMode) {
            contentSize = this.entries.length();
        } else {
            contentSize = this.heapSize;
        }
        assert this.conventionHolds();
        /*
         * RETURN Statement: Returning {@code contentSize}.
         */
        return contentSize;
    }
