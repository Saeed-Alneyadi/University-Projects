...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        /*
         * Extracting the substring of (str1) without the suffix and concatenate
         * it with (result). The resulted string from concatenation is stored to
         * (result). The (result) is also concatenated with (str2) which results
         * to a string stored to (result)
         */
        String result = "";
        result = result + str1.substring(0, str1.length() - overlap);
        result = result + str2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        /*
         * Checks whether the strings inside the Set contains (str). If it is,
         * found is assigned true. Otherwise, false. However, if two strings in
         * the Set contains (str), this will result an ERROR due to violation of
         * the precondition.
         */
        boolean found = false;
        ...
        /*
         * This checks whether (str) contains any of the strings inside the Set.
         * If the string inside the Set is contained, the string will be
         * removed.
         */
        if (!found) {
            temp = strSet.newInstance();
            temp.transferFrom(strSet);
            while (temp.size() > 0) {
                String x = temp.removeAny();
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        /*
         * Set Object Instantiation and Variable Declaration.
         */
        Set<String> strSet = new Set1L<>();
        String in;

        /*
         * Call addToSetAvoidingSubstrings for each line inside (in) and the Set
         * (strSet).
         */
        while (!(input.atEOS())) {
            in = input.nextLine();
            addToSetAvoidingSubstrings(strSet, in);
        }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        /*
         * Prints (text) to the (out) stream where each '~' is a line separator.
         */
        for (int idx = 0; idx < text.length(); idx++) {
            if (text.charAt(idx) == '~') {
                out.println("");
            } else {
                out.print(text.charAt(idx));
            }
        }
