...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        /*
         * Checks whether (m) is not zero. If it is, a new NaturalNumber
         * (remiander) is assigned the remiander of (n) and (m) division,
         * reduceToGCD(m, remainder) is called, and (n) takes the value from (m)
         * where (m) clears.
         */
        if (!(m.isZero())) {
    ...
    public static boolean isEven(NaturalNumber n) {
        /*
         * Variable Initialization and NaturalNumber Instantiation
         */
        boolean even = false;
        NaturalNumber cond = new NaturalNumber2(n);
        NaturalNumber two = new NaturalNumber2(2);

        /*
         * Dividing (cond) by (two).
         */
        cond = cond.divide(two);

        /*
         * Checks whether (cond) is zero. If it is, (even) will be assigned
         * true.
         */
        if (cond.isZero()) {
            even = true;
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        /*
         * NaturalNumber Instantiations
         */
        NaturalNumber factor1 = new NaturalNumber2(n);
        NaturalNumber one = new NaturalNumber2(1);
        NaturalNumber two = new NaturalNumber2(2);

        /*
         * Calculates the power using fast-powering algorithm with the
         * additional feature that every multiplication is followed immediately
         * by "reducing the result modulo m".
         */
        if (p.isZero()) {
            n.copyFrom(one);
        } else {
            NaturalNumber cond = new NaturalNumber2(p);

            cond.divide(two);

            powerMod(n, cond, m);

            NaturalNumber factor2 = new NaturalNumber2(n);
            n.multiply(factor2);

            NaturalNumber mod = new NaturalNumber2(n);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        /*
         * Variables Initializations and NaturalNumber Instantiations
         */
        boolean witness = false;
        NaturalNumber cond1 = new NaturalNumber2(n);
        NaturalNumber cond2 = new NaturalNumber2(n);
        NaturalNumber one = new NaturalNumber2(1);
        NaturalNumber two = new NaturalNumber2(2);
        cond1.subtract(two);
        cond2.subtract(one);
        ...
        /*
         * Check whether it is a prime number or not based on the first fact.
         */
        if ((w.compareTo(two) >= 0) && (w.compareTo(cond1) <= 0)) {
            NaturalNumber mod = new NaturalNumber2(w);
            powerMod(mod, two, n);
        ...
        /*
         * Check whether it is a prime number or not based on the second fact.
         */
        if ((w.compareTo(one) >= 0) && (w.compareTo(cond2) <= 0)) {
            NaturalNumber mod = new NaturalNumber2(w);
            powerMod(mod, cond2, n);
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            /*
             * Generate 50 random number and checks whether these numbers are
            ...
            for (int idx = 0; idx < constant; idx++) {
                ...
                NaturalNumber w = randomNumber(num);
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        /*
         * Finds the likely prime number after (n).
         */
        while (!(isPrime2(n))) {
            if (isEven(n)) {
                n.increment();
