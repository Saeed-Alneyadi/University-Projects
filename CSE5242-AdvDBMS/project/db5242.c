...
inline int64_t low_bin_nb_arithmetic(int64_t* data, int64_t size, int64_t target)
{
  ...
  */
  ...
  while(left<right) {
    ...
    left = (mid+1)*lT + left*gT;
    right = mid*gT + right*lT;
  }
  return right;
}
...
static inline void low_bin_nb_4x(int64_t* data, int64_t size, int64_t* targets, int64_t* right)
{
  /* this binary search variant
  ...
  */
  ...
  while (left[0] < right[0] || left[1] < right[1] || left[2] < right[2] || left[3] < right[3]) {
        for (int i = 0; i < 4; ++i) {
            ...
            gT = data[mid[i]]>=targets[i];
            lT = data[mid[i]]<targets[i];

            // gT = -gT;
            // lT = -lT;

            left[i] = ((mid[i]+1)&(-lT)) + (left[i]&(-gT));
            right[i] = (mid[i]&(-gT)) + (right[i]&(-lT));
        }
    }
}
